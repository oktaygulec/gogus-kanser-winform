...
namespace Kumeleme_Solution_WinForm
{
    public partial class Form1 : Form
    {
        ...
        private void btnYeniVeriEkle_Click(object sender, EventArgs e)
        {
            ...
            Data item = new Data
            { 
                OperasyondakiHastaYasi = xData,
                PozitifAksillerLenfNoduAdeti = yData
            };

            if (cbDurum.Enabled)
                item.HayattaKalmaSuresi = cbDurum.SelectedItem as string;
            else
            {
                Calculate.FindState(item, clusterBS.Count);
                MessageBox.Show(item.HayattaKalmaSuresi);
            }
            ...
            {
                clusterPointChart.Series[item.Cluster].Points.AddXY(item.OperasyondakiHastaYasi, item.PozitifAksillerLenfNoduAdeti);
        ...
        private void btnCSVAc_Click(object sender, EventArgs e)
        {
            OpenFileDialog fileDialog = new OpenFileDialog();
            fileDialog.Filter = "CSV dosyası (.csv)|*.csv";
